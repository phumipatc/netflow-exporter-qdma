			...
			unsigned long arg)
{
	...
	case QDMA_CDEV_IOCTL_NO_MEMCPY:
		...
		return 0;
	case QDMA_CDEV_IOCTL_C2H_PEEK:

		int ret;

		ioctl_c2h_peek_data_t c2h_peek_data;

		// printk("c2h_peek ioctl called\n");

		// Retrieve the structure from user space
		if (copy_from_user(&c2h_peek_data, (ioctl_c2h_peek_data_t *)arg, sizeof(c2h_peek_data))) {
			pr_err("copy_from_user failed for c2h_peek_data.\n");
			return -EFAULT;
		}

		// printk("copied data from user space\n");
		
		// printk("c2h qhndl: %lu\n", xcdev->c2h_qhndl);

		ret = qdma_queue_c2h_peek(
			xcdev->xcb->xpdev->dev_hndl,
			xcdev->c2h_qhndl,
			&c2h_peek_data.udd_cnt,
			&c2h_peek_data.pkt_cnt,
			&c2h_peek_data.data_len
		);

		// printk("qdma_queue_c2h_peek returned %d\n", ret);

		if(ret < 0) {
			pr_err("c2h_peek failed with error code %d\n", ret);
			return -EINVAL;
		}

		// printk("c2h_peek successful\n");

		// Return the updated structure back to user space
		if (copy_to_user((ioctl_c2h_peek_data_t *)arg, &c2h_peek_data, sizeof(c2h_peek_data))) {
			pr_err("copy_to_user failed for c2h_peek_data.\n");
			return -EFAULT;
		}

		// printk("copied data to user space\n");
		...
		size_t count, loff_t *pos, bool write)
{
	...
	printk("cdev_gen_read_write called\n");
